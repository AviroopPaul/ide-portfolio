import { Sparkles, Send, Bot, PanelRightClose, Command, Trash2, X } from 'lucide-react';
    ...
    <>
      ...
      <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <div className="flex flex-col leading-tight">
            <span className="font-bold text-md tracking-[0.2em] inline-block text-[var(--avexa)]">AVEXA</span>
            ...
            <div className={`max-w-[90%] min-w-0 p-2 rounded-lg leading-relaxed break-words ${msg.role === 'user' ? 'bg-accent text-bg font-medium' : 'bg-bg border border-border shadow-sm'}`}>
              {msg.role === 'assistant' ? (
                <div className="text-sm break-words [&_p]:my-1.5 [&_ul]:my-1.5 [&_ol]:my-1.5 [&_pre]:my-2 [&_code]:bg-muted/30 [&_code]:px-1 [&_code]:rounded">
                  <ReactMarkdown components={markdownComponents}>{linkifyRawUrls(msg.content)}</ReactMarkdown>
                </div>
              ) : (
                msg.content
              )}
      ...
      <div className="p-4 pt-0 shrink-0">
        ...
        <div className="mt-2 flex justify-between items-center text-[10px] text-muted/60 px-1 font-medium">
          ...
          {!isMobile && <span className="flex items-center gap-1"><Command size={10} /> + I to toggle</span>}
        </div>
      </div>
    </>
  );
    ...
    return (
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-h-[85vh] rounded-t-2xl bg-card border-t border-l border-r border-border flex flex-col overflow-hidden shadow-2xl"
          style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
        >
          {showModelModal && (
            <div className="absolute inset-0 z-40" onClick={() => setShowModelModal(false)} aria-hidden="true" />
          )}
          {chatContent}
        </motion.div>
      </motion.div>
